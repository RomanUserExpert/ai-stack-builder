...
| **Kind tabs** | My library, Public library, the library panel | `All` first (in the library sidebar: `Related`, then `All` — and **on an empty set the sidebar opens on `All`**, since nothing is there yet for `Related` to relate to), then one tab per kind in §5's order — `skill · agent · prompt · mcp · script · app`. **On a page they carry counts** (owner, 2026-09-26), **and the counts follow the search**, so a tab with matches is itself a way out. A zero is shown, never hidden, so the order holds. Kind is a tab, not a filter — `Filters` holds the rest |
...
| **Set list** | Project, Configuring | **The Library's row, in a set** (owner, 2026-09-26): name · kind · **state badges** (*Auto-added*, *Detached*, *Conflict* — a dashed tag) · one line of description · one grey line of facts. **Findings from the last check are a mark** — a glyph and a word, *Problem* or *Note* — in a narrow middle column. **Auto-added items nest under the item that pulled them in** — a row with sub-rows and a guide line (owner, 2026-09-26); the parent's facts say *brings 3 items with it*. In Configuring the right column carries the remove, or the detached row's commands; **a sub-row has no remove of its own** — removing the parent takes it along, and the nesting is what says so |
| **Side panel** | Project — `project-item.html`; Configuring — `project-configuring-item.html`, `project-detached-*.html` | **Clicking a row opens the item beside the list, not over it** — a column on the right, the set still in view, the selected row filled grey. **Viewing: read-only** (*Open in My library*, *Remove in Configure*). **Configuring: where a row changes** — a linked item offers **Detach to edit here**; a detached item shows the differing fields, editable, with *Reset this field*, *Reset whole item* and *Promote to My library…*; its edits are kept with the project's *Save*. Drawn as its own page, like the filters popover |
