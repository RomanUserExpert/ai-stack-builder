...
| **Kind tabs** | My library, Public library, the library panel | `All` first (in the library sidebar: `Related`, then `All`), then one tab per kind in §5's order — `skill · agent · prompt · mcp · script · app`. **On a page they carry counts** (owner, 2026-09-26), **and the counts follow the search**, so a tab with matches is itself a way out. A zero is shown, never hidden, so the order holds. Kind is a tab, not a filter — `Filters` holds the rest |
