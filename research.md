...
[`research/FINAL.md`](research/FINAL.md), which records the decisions and the closed questions.
Nothing here is a third list of open questions — the register lives in
[`research/research-plan.md`](research/research-plan.md) and is currently empty.
...
**The evidence base, counted rather than estimated.** 143 image captures, 36 written documents and
7 raw data logs under [`research/`](research/). Stage 1's 38 are catalogued in
[`research/1-landscape/screens-index.md`](research/1-landscape/screens-index.md) with sign-in walls
labelled; the rest sit in their stage folders. Two public issue trackers were queried through the
GitHub API, raw results in
[`research/3-pain/_user-pain-issues.json`](research/3-pain/_user-pain-issues.json).
...
[`research/1-landscape/comparison.md`](research/1-landscape/comparison.md), built from the products
themselves, with the per-company entries and verified links in
[`research/1-landscape/competitors.md`](research/1-landscape/competitors.md).

## The matrix — fifteen products on five axes

Cells are condensed; the full text is in
[`comparison.md`](research/1-landscape/comparison.md). *Audience* — who it is sold to.
...
Captures: [`skill-detail-scored.png`](research/1-landscape/screens/hard/tessl/skill-detail-scored.png)
(the reference screen of the survey — composite 93, uplift 1.40×, Quality/Impact/Security bars, repo
+ path + commit) ·
[`server-detail-exa.png`](research/1-landscape/screens/hard/smithery/server-detail-exa.png) ·
[`acquisition-notice.png`](research/1-landscape/screens/hard/continue/acquisition-notice.png) ·
[`marketing-home.png`](research/1-landscape/screens/hard/packmind/marketing-home.png) ·
[`marketing-skills-library.png`](research/1-landscape/screens/hard/agentman/marketing-skills-library.png).
...
> [`continue-postmortem.md`](research/1-landscape/continue-postmortem.md) — which is the only place
...
Captures:
[`catalog-relations-graph.png`](research/1-landscape/screens/soft/backstage/catalog-relations-graph.png) ·
[`catalog-table.png`](research/1-landscape/screens/soft/backstage/catalog-table.png) ·
[`port-scorecard-rule-expanded.png`](research/2-flows/04-validation-check-results/port-scorecard-rule-expanded.png) ·
[`module-dependencies.png`](research/1-landscape/screens/soft/terraform/module-dependencies.png) ·
[`template-gallery.png`](research/1-landscape/screens/soft/notion/template-gallery.png).
...
Captures:
[`vercel-deployment-stages.jpg`](research/2-flows/04-validation-check-results/vercel-deployment-stages.jpg) ·
[`linear-command-palette-default.jpg`](research/2-flows/02-library-browse-filter-search/linear-command-palette-default.jpg) ·
[`geist-colors.png`](research/1-landscape/screens/aspirational/vercel/geist-colors.png) ·
[`api-reference.png`](research/1-landscape/screens/aspirational/stripe/api-reference.png) ·
[`dependency-graph-nextjs.png`](research/1-landscape/screens/aspirational/github/dependency-graph-nextjs.png).
...
> [`screens-index.md`](research/1-landscape/screens-index.md). Linear and Vercel were later captured
...
1. **The composition layer is consolidating, fast.** Of five hard competitors, one had its hosted
   ...
   were left standing.** Source: [`comparison.md`](research/1-landscape/comparison.md),
   [`continue-postmortem.md`](research/1-landscape/continue-postmortem.md), capture
   [`acquisition-notice.png`](research/1-landscape/screens/hard/continue/acquisition-notice.png).
...
2. **Trust has moved from social proof to measurement.** Tessl scores every skill on quality, impact
   ...
   **A number derived from *running* the thing has replaced stars and download counts.** Source:
   [`comparison.md`](research/1-landscape/comparison.md), captures
   [`skill-detail-scored.png`](research/1-landscape/screens/hard/tessl/skill-detail-scored.png),
   [`tessl-skill-evals-tab.png`](research/2-flows/01-item-detail-and-trust/tessl-skill-evals-tab.png).
...
3. **Nobody authors a graph; everybody derives one.** Backstage's Catalog Graph is read-only,
   ...
   Source: [`comparison.md`](research/1-landscape/comparison.md), captures
   [`backstage-graph-depth-1.png`](research/2-flows/03-relations-without-canvas/backstage-graph-depth-1.png) and
   [`backstage-graph-depth-3.png`](research/2-flows/03-relations-without-canvas/backstage-graph-depth-3.png)
...
All three: [`comparison.md`](research/1-landscape/comparison.md).
...
> ([`continue-postmortem.md`](research/1-landscape/continue-postmortem.md)). Beyond that —
...
[`research/2-flows/README.md`](research/2-flows/README.md). Ten closed, one declined, one handed
forward.

| # | Flow | Status | The insight it produced | Source |
|---|---|---|---|---|
| **01** | Item detail and trust | ◐ declined | Trust has moved from social proof to **measurement** — a number derived from running the thing. We can run nothing, so we ship none of it | [`tessl-skill-evals-tab.png`](research/2-flows/01-item-detail-and-trust/tessl-skill-evals-tab.png), [`tessl-skill-quality-tab.png`](research/2-flows/01-item-detail-and-trust/tessl-skill-quality-tab.png), [`smithery-server-github-config.png`](research/2-flows/01-item-detail-and-trust/smithery-server-github-config.png) |
| **02** | Library browse, filter, search | ● closed | Typing **flattens a filter tree into breadcrumbs** — the path is shown, not traversed, which is how two leaves with the same name stay distinguishable. A filter chip is a sentence. And **name how many items are hidden**, never claim there are none: *"4 issues hidden by filters · Clear Filters ✕"* | [`NOTES-linear.md`](research/2-flows/02-library-browse-filter-search/NOTES-linear.md), [`linear-filtered-to-zero-hidden-count.jpg`](research/2-flows/02-library-browse-filter-search/linear-filtered-to-zero-hidden-count.jpg), [`linear-filter-typeahead-breadcrumbs.jpg`](research/2-flows/02-library-browse-filter-search/linear-filter-typeahead-breadcrumbs.jpg) |
| **03** | Relations without a canvas | ● closed | **Nobody authors a graph.** It is derived from declared relations and made legible with a depth filter. GitHub's *Dependents* is the reverse direction — blast radius rather than dependencies | [`backstage-graph-depth-1.png`](research/2-flows/03-relations-without-canvas/backstage-graph-depth-1.png), [`backstage-graph-depth-3.png`](research/2-flows/03-relations-without-canvas/backstage-graph-depth-3.png), [`github-dependents-blast-radius.png`](research/2-flows/03-relations-without-canvas/github-dependents-blast-radius.png) |
| **04** | Validation: pass, warn, block | ● closed | A check is a **stack of stages, each with its own verdict and duration**, expandable. And **Port does not block — it gates a level**: a failed rule stops an entity climbing `Basic → Low → Good → Great` rather than forbidding anything. The demo has no blocking surface at all | [`NOTES-vercel.md`](research/2-flows/04-validation-check-results/NOTES-vercel.md), [`NOTES-port.md`](research/2-flows/04-validation-check-results/NOTES-port.md), [`NOTES-github-actions.md`](research/2-flows/04-validation-check-results/NOTES-github-actions.md), [`terraform-plan-output.md`](research/2-flows/04-validation-check-results/terraform-plan-output.md), [`vercel-deployment-stages.jpg`](research/2-flows/04-validation-check-results/vercel-deployment-stages.jpg), [`port-scorecard-rule-expanded.png`](research/2-flows/04-validation-check-results/port-scorecard-rule-expanded.png) |
| **05** | Linked vs detached, blast radius | ● closed | **Drift is computed precisely and displayed nowhere.** Figma's menu names the exact overridden property — `Reset fill`, not "reset overrides" — yet a modified instance is pixel-identical to a clean one in the layers tree, the properties panel and the canvas. **The failure is display, not modelling.** Also: revert at two granularities, and Reset kept next to Detach as two halves of one axis | [`NOTES.md`](research/2-flows/05-linked-vs-detached/NOTES.md), [`figma-context-menu-overridden-reset.png`](research/2-flows/05-linked-vs-detached/figma-context-menu-overridden-reset.png), [`figma-context-menu-clean.png`](research/2-flows/05-linked-vs-detached/figma-context-menu-clean.png), [`figma-library-updates-instance-counts.png`](research/2-flows/05-linked-vs-detached/figma-library-updates-instance-counts.png) |
| **06** | Export and target adaptation | ● closed | **One source, many targets** — and external repos are always *instructions*, never vendored. Ruler emits a managed `START/END` fenced block so a re-run replaces exactly what it wrote, and drops a `.bak` beside every overwritten file | [`ruler-per-agent-output.md`](research/2-flows/06-export-and-target-adaptation/ruler-per-agent-output.md), [`backstage-scaffolder-templates.png`](research/1-landscape/screens/soft/backstage/scaffolder-templates.png) |
| **07** | Env variables and secrets | ● closed | **Ask the type before the value**, default to the irreversible option (Secret, not Config), and let the placeholder pose the real question — Vercel's note field reads *"Where to rotate, or who to contact"* | [`NOTES.md`](research/2-flows/07-env-and-secrets/NOTES.md), [`vercel-add-env-variable-drawer.jpg`](research/2-flows/07-env-and-secrets/vercel-add-env-variable-drawer.jpg) |
| **08** | Empty state and cold start | ● closed | **Answer an empty product with real objects, not an empty state.** A new Linear workspace opens on four *real* issues — editable, completable, deletable — so the model is learned by holding four instances of it. And **three registers of emptiness**: never-used concept → define it; routine → one line; filtered to zero → count what is hidden | [`NOTES-linear.md`](research/2-flows/08-empty-state-and-cold-start/NOTES-linear.md), [`linear-first-run-seeded-issues.jpg`](research/2-flows/08-empty-state-and-cold-start/linear-first-run-seeded-issues.jpg), [`linear-projects-empty-state.jpg`](research/2-flows/08-empty-state-and-cold-start/linear-projects-empty-state.jpg) |
| **09** | Duplicate and fork | ● closed | **Duplication is a distribution primitive**, and the copy dialog states what will and will not come along | [`NOTES.md`](research/2-flows/09-duplicate-and-fork/NOTES.md), [`github-create-fork-form.jpg`](research/2-flows/09-duplicate-and-fork/github-create-fork-form.jpg), [`notion-page-menu-duplicate.jpg`](research/2-flows/09-duplicate-and-fork/notion-page-menu-duplicate.jpg) |
| **10** | Dark design language | → handed forward | Few surfaces with the sidebar sharing the content ground; elevation as a hairline plus a few percent of lightness rather than a shadow ramp; three foreground tones with the accent reserved for meaning; key caps as a real component. **Deliberately not spent in this phase** — it is the one flow about appearance, and it opens the design-system phase | [`NOTES-linear.md`](research/2-flows/10-dark-design-language/NOTES-linear.md), [`linear-command-palette-dark-detail.png`](research/2-flows/10-dark-design-language/linear-command-palette-dark-detail.png) |
| **11** | Copy: errors, warnings, refusals | ● closed | **Cause and consequence are two rows**, and you group by the action that fixes a failure, not the check that found it. The anti-reference is industry-wide: 14 of 17 annotations on one run read `Process completed with exit code 1`, and the same was found in `denoland/deno`, `withastro/astro`, `vitejs/vite` and `rust-lang/rust-clippy` | [`ci-failure-copy.md`](research/2-flows/11-copy-and-error-language/ci-failure-copy.md), [`dependency-conflict-copy.md`](research/2-flows/11-copy-and-error-language/dependency-conflict-copy.md), [`github-actions-run-failed-annotations.png`](research/2-flows/04-validation-check-results/github-actions-run-failed-annotations.png) |
| **12** | Visibility and portfolio | ● closed | **Public/private is a low-ceremony decision**, and the profile is the portfolio | [`NOTES.md`](research/2-flows/12-visibility-and-portfolio/NOTES.md), [`github-danger-zone-visibility.jpg`](research/2-flows/12-visibility-and-portfolio/github-danger-zone-visibility.jpg), [`notion-publish-to-web.jpg`](research/2-flows/12-visibility-and-portfolio/notion-publish-to-web.jpg) |
...
- **Density under load.** Every browse mechanic in flow 02 was captured in a workspace holding
  **four issues**, and the benchmark's Obsidian cell against a **27-note** vault. All of it is
  ...
  ([`README.md`](research/2-flows/README.md), *the trade, stated plainly*.)
...
[`research/4-benchmark/benchmark.md`](research/4-benchmark/benchmark.md).
...
**Source.**
[`NOTES-vercel.md`](research/2-flows/04-validation-check-results/NOTES-vercel.md) ·
[`vercel-deployment-stages.jpg`](research/2-flows/04-validation-check-results/vercel-deployment-stages.jpg) ·
[`vercel-build-log-timestamp-tooltip.jpg`](research/2-flows/04-validation-check-results/vercel-build-log-timestamp-tooltip.jpg) ·
[`terraform-plan-output.md`](research/2-flows/04-validation-check-results/terraform-plan-output.md) ·
[`benchmark.md`](research/4-benchmark/benchmark.md), B3.
...
**Source.**
[`NOTES-vscode.md`](research/4-benchmark/NOTES-vscode.md) ·
[`vscode-workspace-trust.png`](research/4-benchmark/vscode-workspace-trust.png) ·
[`vscode-problems-filtered-zero.png`](research/4-benchmark/vscode-problems-filtered-zero.png) ·
[`linear-filtered-to-zero-hidden-count.jpg`](research/2-flows/02-library-browse-filter-search/linear-filtered-to-zero-hidden-count.jpg) ·
[`benchmark.md`](research/4-benchmark/benchmark.md), findings 4 and 5.
...
**Source.**
[`npm-eresolve.md`](research/4-benchmark/npm-eresolve.md) ·
[`NOTES-port.md`](research/2-flows/04-validation-check-results/NOTES-port.md) ·
[`port-scorecard-rule-expanded.png`](research/2-flows/04-validation-check-results/port-scorecard-rule-expanded.png) ·
[`NOTES-github-actions.md`](research/2-flows/04-validation-check-results/NOTES-github-actions.md) ·
[`github-actions-run-failed-annotations.png`](research/2-flows/04-validation-check-results/github-actions-run-failed-annotations.png) ·
[`ci-failure-copy.md`](research/2-flows/11-copy-and-error-language/ci-failure-copy.md) ·
[`benchmark.md`](research/4-benchmark/benchmark.md), findings 1 and 2.
...
**Source.**
[`NOTES-figma-export.md`](research/4-benchmark/NOTES-figma-export.md) ·
[`figma-export-empty-selection.png`](research/4-benchmark/figma-export-empty-selection.png) ·
[`NOTES-port.md`](research/2-flows/04-validation-check-results/NOTES-port.md) ·
[`benchmark.md`](research/4-benchmark/benchmark.md), finding 7 · [`CLAUDE.md`](CLAUDE.md) §6.
...
([`user-pain.md`](research/3-pain/user-pain.md)) and the weights come out uneven:
...
Sources: [`benchmark.md`](research/4-benchmark/benchmark.md) finalisation ·
[`user-pain.md`](research/3-pain/user-pain.md) ·
[`_user-pain-issues.json`](research/3-pain/_user-pain-issues.json) ·
...
asking for a composition layer ([`user-pain.md`](research/3-pain/user-pain.md), finding 4; and the
...
Source: [`patterns.md`](research/5-patterns/patterns.md), *What this costs us, said plainly*.
