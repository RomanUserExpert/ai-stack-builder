...
> [`interviews.md` the guide](../6-personas/interviews.md#part-1-the-guide) and filed as source documents. **One has
...
> [`re-research.md` round 3](../6-personas/re-research.md#round-3-five-questions-that-need-no-interview), and §11 says which cells it could move.
...
> **H-J4's P3**, from [`re-research.md` round 2](../6-personas/re-research.md#round-2-counting-not-asking) Q-E — **and the evidence
...
- `*` [interview Q5](../6-personas/interviews.md#part-2-interview-1-of-5) — four MCP servers silently not starting on Linux, twenty minutes spent believing *the model had gotten worse*; [Q11](../6-personas/interviews.md#part-2-interview-1-of-5) — Node 18 against a server needing 20+, *"broken config doesn't announce itself, **it degrades quietly**"*.
...
**Grew from.** `*` [interview Q21](../6-personas/interviews.md#part-2-interview-1-of-5) — a contractor got the repo,
...
`*` [interview Q13](../6-personas/interviews.md#part-2-interview-1-of-5) — a formatting rule carried in from another
...
[interview Q10](../6-personas/interviews.md#part-2-interview-1-of-5) — the same off-by-one fixed on a Friday in one
...
[interview Q23](../6-personas/interviews.md#part-2-interview-1-of-5) — he cannot share any of it because
...
[interview Q13](../6-personas/interviews.md#part-2-interview-1-of-5) — *"it touches migrations. Not always, and
...
· `*` [interview Q16, Q18](../6-personas/interviews.md#part-2-interview-1-of-5) — *"maybe **half of it**, if you
...
`*` [interview Q21](../6-personas/interviews.md#part-2-interview-1-of-5) — *"that was the moment I understood the
config had become **tribal knowledge rather than a setup**"* · `*`
[Q22](../6-personas/interviews.md#part-2-interview-1-of-5) — what he had to explain, twice, on a call: *"which
...
`*` [interview Q23](../6-personas/interviews.md#part-2-interview-1-of-5) — *"I'd like a slice of it public, mostly
...
**and after the audit all three are 2s, every one of them second-hand** — *no longer true as of 2026-09-10: **RJ-4/P2 and SJ-1/P2 are now 2s resting on observed diffs**, which is behaviour rather than somebody's account. The three the sentence was written about are unchanged* — because only one issue's
...
### What round 4 did to this matrix — 2026-09-10, and it is the first round that adds

**[`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) went after the persona columns directly**, in
two venues this repository had never used — **[forum.cursor.com](https://forum.cursor.com)** and
**[community.openai.com](https://community.openai.com)**, both Discourse, both with the **vendor
answering in public**. That last part is new: several findings below are a vendor engineer confirming
a defect, which is a different class of source from a user reporting one.

**Three cells move, and all three move up.** The audit of 2026-09-09 only ever subtracted and round 3
moved nothing vertical; this is the first upward movement in the matrix's history.

| Cell | Was | Is | On what |
|---|---|---|---|
| **RJ-2 / P1** | 2 | **3** | Skills loaded from four roots at once, **duplicates confirmed by the vendor not to collapse**, no fix — and **two people rebuilt their tooling** rather than live with it (`chflags uchg` purge script; a private mount namespace over the skill roots). The audit's own test for a 3, met |
| **EJ-2 / P1** | `[?]` | **2** | A named person with a **reproduction repository** and ten days spent, asking to *"inspect the raw full context window… to tell if it is an issue with AI not loading the rule vs just deciding not to follow them"*, with the vendor confirming rules apply inconsistently |
| **H-J3 / P1** | `[?]` | **2** | *"I need share the rules with my team… **each user should be able to slightly modify the rules**"* — `detached` plus `overrides`, stated as a requirement by somebody blocked three ways. **J-107 said this stood on nothing** |

**And two things that did not move, for reasons worth more than the cells.**

- **P2's seven `[?]` stayed `[?]` in the forum pass, and its three 2s stayed second-hand** — **and then the same day the fork corpus filled two of them with behaviour** (F9, F10 below). Handover to a team is asked
  about repeatedly — four independent topics on Cursor's forum, two of them at ~1,380 views, the
  official answer *"commit the rules inside of your Repo"* — **and every one of those askers is a
  sender.** After four rounds and five venues, **no receiver has spoken in the first person.** That is
  now a finding about the population rather than about the search: the person who receives has no
  reason to post, because the thing they received is not theirs.
- **P3's ten `[?]` stay `[?]`, and the two clearest public beginners did the opposite of what the
  shelf assumes.** One *"created a detailed Markdown file with all the info about the project"* on
  day one; the other asked *"if anyone experienced here would be willing to let me shadow them as
  they code for 30 mins"*. One made his own, one wanted a person. **Neither reached for a library.**
  Both threads got **no reply at all.**

**A second pass the same day found four more cells, and two of them are in P2.** The first pass concluded the receiver never speaks, which is true — and it is not the only way to reach one. **Round 3 had already read 214 forks with their patches**, and nobody had asked that corpus about the matrix. A receiver who never posts still leaves a diff.

| Cell | Was | Is | On what |
|---|---|---|---|
| **RJ-3 / P1** | 2 | **3** | The job stated verbatim with its cost — *“the risk of forgetting to update a copy, leading to inconsistencies in agent behavior across projects”* — by somebody who then **tried five mechanisms and failed at all five**. The audit lowered this cell because its one person changed nothing; this one changed everything, five times |
| **H-J1 / P1** | `[?]` | **2** | A Cursor user whose prompt library **is a folder of notes in Telegram**, re-entering every file and skill link by hand, who checks whether skills solve it and answers himself *“No, that's not it”* |
| **RJ-4 / P2** | `[?]` | **2** | **Four receivers watched stripping the author's credentials out of inherited material**, one in a commit titled *“remove encrypted files”*, one in a commit titled *“Re-committing work without secrets”* who then wrote the `.env.example` the material lacked |
| **SJ-1 / P2** | `[?]` | **2** | **30 of 214 receivers wrote 21,266 lines** of setup, handover and installation manual the sender did not ship — `HANDOFF.md` at 229 lines as a fork's only commit, `SETUP.md` at 425, `docs/windows-setup.md` at 284 |

**A third pass added one more and stopped.** **H-J7/P1 `[?]` → 2** — the unwritten half, four independent people saying it in the first person, one pricing it at *“10–30 minutes every time”* you switch tools. **Not a 3, and the reason is ours rather than theirs**: all four were found in a venue where having built something is the entry ticket, so counting the tools they built as importance would be circular. **Q11 no longer stands on one aside.**

**And P3 was hunted a fourth time by inverting the method** — go to the shelves rather than to the people: 18 of the largest public collections, 568 issues. **Three real consumers, blocked by exactly what §5's `requires` and §6's dependency walk exist for**: a 4,386★ collection whose agents need an **undeclared `context7 MCP`**, so *“new users… are unable to run these agents as intended”*; a first-timer meeting a per-file collision prompt he cannot answer, *“Is this supposed to happen? What should i do here?”*; and one lost between several installed agents. **Not one of them fills a cell.** **The blocker on what remains is persona attribution, not evidence** — a public artefact shows an act, and a persona is defined by a situation. **P2's five and P3's ten are waiting for somebody to be asked, not for a better query.**

**And RJ-1 / P1 was hunted on purpose and is still `[?]`.** Six title queries aimed straight at it — *what to include*, *checklist*, *requirements to run*, *works on my machine*, *missing dependency* — across both forums, and **nobody in five venues says they wished they had known what the other side would need.** §8 calls the gap here the widest in the file; this makes it wider. **And since RJ-3 has now moved to a 3, §8's line about RJ-1 overtaking it needs re-reading at the sitting.**

**Two findings land outside the matrix and belong to the register.** **Q12** — observability rather
than validation — **no longer stands on one person**; EJ-2's new evidence is the sharpest public
statement of it anybody has made. And the market's managed answer to distributing a **set** turns out
to be a paid tier: the vendor tells a user to *"wrap the skill as a plugin in the Team Marketplace and
mark it as Required so it installs for everyone automatically… only available on Teams/Enterprise"*,
and the user answers *"Huge unlock."*

### What round 3 did to this matrix — 2026-09-10, and the answer is *no number moved*

**[`re-research.md` round 3](../6-personas/re-research.md#round-3-five-questions-that-need-no-interview) ran five questions and changed not one
importance in the ten rows above.** That is worth stating first and plainly, because the round
produced the strongest evidence in the folder and it still did not move a single cell vertically.
**The reason is structural: the columns of this matrix are people, and round 3's instruments see
machines and artefacts.** A receiving agent that performs a setup is not P2. A repository whose two
copies of one skill have differed for four months tells us the drift is real and nothing about what
it cost. An importer who edits a vendored skill four weeks later is a behaviour without a person
attached. **Importance is a fact about somebody, and round 3 asked nobody anything.**

**What did change is the two right-hand columns, in nine cells**, and the audit's own convention is
followed — every change is dated in the cell and says what it rests on.

| Kind of change | Cells |
|---|---|
| **A market cell falsified in part.** *Not one product says anything about the machine its artefact lands on* was true of funded vendors in August 2026 and is **not true now**: `asm doctor` is a B4 surface in a 916★ tool | **MAIN**, **RJ-1**, **SJ-1** |
| **A vendor claim turned into a measurement, and it failed.** `asm`'s duplicate audit, run on our set, misses a whole agent root and calls two files with different sha1s *"identical copies"* | **RJ-2**, and it lands beside Continue in **EJ-1** |
| **A feature cell tested rather than asserted.** Three receiving agents performed the setup from `SETUP.md`; none was told what was wrong with the set, and two then mis-resolved or missed every defect | **MAIN**, **RJ-1**, **SJ-1** |
| **A premise measured under an existing number, and the number survives because of it.** Drift is real at 14% of 7,506 duplicated items and lasts a median of 121 days — **and nobody reconciles**, which is exactly why RJ-3's P1 stays a 2 and not a 3 | **RJ-3**, **H-J3** |
| **A cell that had opinion against it now has behaviour for it.** Thousands of repositories carry verbatim public skills; 32% of 446 traced copies were edited after import; 10% keep provenance | **H-J4** |
| **A standing improved without a number changing.** Every P2 number in this matrix was second-hand; **25 receivers were now watched doing the work in public diffs** | **RJ-1/P2** |
| **And then two P2 cells filled, on behaviour.** The receiver still never speaks — but the fork corpus **shows** them: four stripping the author's credentials out of inherited material, and **30 of 214 writing 21,266 lines of setup and handover manual** the sender did not ship | **RJ-4/P2 · SJ-1/P2** |
| **A risk found running the other way down the pipe.** A receiving agent filled a named-but-empty env key by taking the machine's live credential out of its keyring | **RJ-4** |

**And one thing the round nearly did to this matrix and did not.** Q-G's headline — 89.4% of forks of
personal agent material carry no commit — was drafted as evidence that *the archive is not being
received at all*, which would have forced a rewrite of the whole P2 column. **Its control came back
at 84.8% and 87.0% for populations with nothing to do with agents**, so the dormancy is a fact about
forks and the reading was withdrawn before it reached this file. **The P2 column is unrewritten, and
that is a result rather than an omission.**

## The sourced jobs

| Job | **P1** keeper · *primary* | **P2** receiver | **P3** empty-handed | Feature that closes it | Do competitors close it? |
|---|---|---|---|---|---|
| **MAIN — make it keep working somewhere else** | **3** — `✓` [#6235](https://github.com/anthropics/claude-code/issues/6235) **6,592**, the loudest demand anywhere in the base, and [#64](https://github.com/modelcontextprotocol/servers/issues/64) **182**, the loudest pain in the older corpus · `*` Q5, Q11 — twenty-five minutes to detect, two to fix | **2** — *lowered from 3, 2026-09-09, J-59.* `✓` #6235's stated motive **is** this job: *"it doesn't work as well when collaborating with developers who aren't using Claude Code"* — **but its filer is an author**, and the only receiver ever described *worked around it for two days without mentioning it*. That is *it costs them something*; it is not *a reason they would change how they work* · `*` Q21, **second-hand** | **`[?]`** — someone with nothing has nothing to move. Plausible is not evidence | **§6** — the agent target selector, `SETUP.md` written for the receiving agent, pinned `ref`s, target-correct paths · **§8** — the handover stages before Export | **No.** `V` [`benchmark.md`](../4-benchmark/benchmark.md) — **B4 is the weakest flow in the industry, nobody above 4**, and not one of fifteen cells scores what a product says about the machine its artefact lands on. Per-agent *output* exists (Ruler, `create-next-app`, flow 06) and skill managers **claim** per-agent sync in their READMEs (R8, unrun) — **the item-level half is contested, the set-level half is open**. **Narrowed 2026-09-10 (round 3, Q-J), and this is the sharpest correction the round makes to the matrix:** *not one cell scores what a product says about the receiving machine* **is no longer true.** `asm doctor` — installed and run — reports git presence and version, `gh` presence **and authentication**, Node version, **21 provider directories writable**, lock-file integrity, registry reachability, disk space and **PATH shadowing**. That is a B4 surface in a 916★ tool. **The set-level half is still open**: four tools were run and none has a set-level unit at all. And the **feature** cell is no longer an assertion — Q-F ran it, and three receiving agents out of three performed the setup |
| **RJ-1 — know what the other side needs, before I send it** | **`[?]`** — *the number was withdrawn 2026-09-09, J-61.* The cell held a **2** on evidence it described in the same breath as an inference: `*` Q21 only, plus `V` the B4 gap. **He found out on a call and never said he wanted to know beforehand**, and the rule above says an importance nobody has told us is `[?]` | **2** — *lowered from 3, 2026-09-09, J-62.* `*` Q21: *"he assumed that was normal and **worked around it for two days without mentioning it**"* — two days lost is *it costs them something*, and **he changed nothing**, which is the opposite of a 3 · `✓` #6235's motive, filed by an author. **Second-hand — the contractor was never asked** | **`[?]`** | **§6** — `SETUP.md` per item in the resolved set, `.env.example`, pinned `ref`s · **§8** — the last stages of Run, read before the irreversible step | **No, and it is still the clearest open cell — but the structural reason is gone.** *Updated 2026-09-10, round 3 Q-J.* This read *no candidate has a surface that describes the receiving machine at all*, and **`asm doctor` is one**. What no candidate has is a surface describing the receiving machine **for a named set**, which is a narrower and more defensible claim. **And the P2 cell's standing improved without its number changing:** round 3 Q-G watched 25 receivers, in public diffs, doing the work this job is about — rewriting setup URLs, git identity, absolute paths and branch conventions to be their own. **That is behaviour, where every P2 number in this matrix was second-hand.** It corroborates a **2** — it cost them work — and it does not reach a **3**, because none of them said anything and the audit's rule holds |
| **RJ-2 — what my pieces drag in, and where two will fight** | **3** — *raised from 2 on 2026-09-10 by [`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) F1, and it is the first upward move any round has made in this matrix.* `✓` [Cursor forum #160677](https://forum.cursor.com/t/excessive-token-usage-cursor-auto-loads-too-many-skills-from-claude-skills-at-conversation-start/160677), 571 views, open: skills are loaded from four roots at once and **duplicates do not collapse**, which **the vendor confirms in public** — *“normalization works for `~/.claude/skills` vs `~/.cursor/skills`, but GStack stores skills in `~/.agent/skills` (no `s`)… that’s why duplicates don’t collapse… no fix yet”*. **Two people changed how they work rather than live with it**: one purges a folder and marks it immutable with `chflags uchg` after every update, one re-execs `cursor-agent` inside a private mount namespace with bind-mounts over the skill roots. That is the audit’s own test for a **3**, met. It also carries **X6** independently — *“agents spread their focus onto irrelevant built-ins… even when the task has nothing to do with that”* — which had stood on one interview sentence. Prior evidence stands: `✓` [#1219](https://github.com/modelcontextprotocol/servers/issues/1219) **13**, our own thesis sighted in the exact file we generate, plus the *set-and-ignored* family at 6, 3, 2, 0, 0, 0 (OBS-17) · `*` Q13 — 1,100 lines of noise into a colleague's branch. **13 against 182 is *quieter*, and that is all the count says** | **`[?]`** — a receiver does not assemble the set; whether they inherit its collisions is unestablished | **`[?]`** | **§6** — the dependency walk, `conflicts`, duplicate command names, same-target-path, cycles reported as information · **§8** — Run as a stage list | **Partly, and never for this material.** `E` the nearest dead competitor **computed the duplicate correctly and discarded the loser in silence** — `BlockDuplicationDetector`, then `mergeUnrolledAssistants`, no error, no warning (postmortem §5). `M` npm `ERESOLVE` and `terraform validate` do it excellently **in their own domains**. ~~`V` `asm`'s README claims duplicate and near-duplicate audit and names *trigger collision* — unrun~~ **Run on 2026-09-10 (round 3, Q-J), and the result belongs beside Continue's.** Given the `pdf` skill copied into `.claude/`, `.codex/` and `.cursor/`, with the Codex copy edited: `asm` **does not scan `.cursor/skills/` at all**, so the modified copy is invisible; and on the two it does see — 8,072 bytes against 8,088, different sha1, a visibly different `description:` line — it reports **`✓ identical copies`** and offers **`asm audit -y` to auto-remove duplicates**. **The claim is `✓` as a feature and refuted as a capability**, on our own set, dated and versioned. The nearest dead competitor discarded the loser in silence; the nearest live one does not compute the difference in the first place |
| **RJ-3 — fix it once and have the fix reach every copy** | **3** — *raised on 2026-09-10 by [`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) F7, on a second person who did what the first did not.* `✓` [Cursor forum #167666](https://forum.cursor.com/t/question-about-syncing-plugin-changes-across-projects-local-development/167666), 8 replies: **the job stated verbatim, with its cost** — *“I'm forced to **duplicate configuration files (rules and skills) across projects manually**… when I need to refine a rule in one project, I have to simultaneously apply the same changes to all the others… **risk of forgetting to update a copy, leading to inconsistencies in agent behavior across projects**”*. **He then tried five mechanisms and every one failed**: the plugin installer pins to the first commit, the local plugin folder has no per-project on/off, two marketplace update paths *“nothing would actually update”*, and the symlink route dies on `.gitignore` — *“if they're in .gitignore, Cursor doesn't see them”*. **Rebuilding your tooling five times is the audit's own test for a 3.** Round 3's Q-H measured the state underneath: 14% of 7,506 duplicated items out of sync, 383 divergences open at a median of 121 days. *The 2 below was correct on the evidence it had:* `✓` [#9444](https://github.com/anthropics/claude-code/issues/9444) **48**: *"maintenance burden… **inconsistency risk — copies can drift out of sync**"* · `*` Q10 — the same off-by-one fixed in one of three projects, **found six weeks later by a client, three hours to re-debug**. The price is real and the behaviour is not: **he still moves material by `cp -r`** (Q7) | **`[?]`** — whether a receiver cares about the author's other copies is unestablished | **`[?]`** | **§5** — an item lives in the library once and projects link to it **live**. The one mechanism in the spec that exists for exactly this job | **Solved elsewhere, dead here.** `M` Figma does it well for components, down to *423 instances* on one row (flow 05). `E` Continue's hub did it for **this** material and was switched off. `V` `HarnessKit`'s README claims per-agent drift detection — **still unrun, and now for a stated reason: it is not on npm under that name** (the package `harnesskit` belongs to a different project), so round 3 could not install it. **What round 3 did do is measure the premise under this row** (Q-H): across 500 published trees, **14% of 7,506 duplicated items are out of sync right now**, and over history **383 divergences are open at HEAD with a median age of 121 days** while **four ever closed, all within seven hours**. **The P1 number stays 2 and the measurement is the reason it stays 2** — the audit lowered this cell because the person on record kept moving material with `cp -r`, and the public record now says the same thing at scale: **people do not reconcile.** The drift is real; the change of behaviour is not |
| **RJ-4 — move the work without moving the secrets** | **2** — *lowered from 3, 2026-09-09, J-68; the superlative corrected with it.* `✓` **the largest crowd on a fear, after the multi-target family** — *largest in the corpus* was wrong, the R1 family (6,592 · 1,416 · 624 · 472) is larger by any measure — **across three trackers**: [#32733](https://github.com/anthropics/claude-code/issues/32733) 192 · [#401](https://github.com/anthropics/claude-code/issues/401) 54 · [#29910](https://github.com/anthropics/claude-code/issues/29910) 45 · [continue #1729](https://github.com/continuedev/continue/issues/1729) 32 · [#1018](https://github.com/modelcontextprotocol/servers/issues/1018) 23 · [#754](https://github.com/modelcontextprotocol/servers/issues/754) 22 · `*` Q23 — the private and the reusable are **tangled in the same files**, and **he has not done the separation**: *"never the most urgent couple of hours"*, which is a 2 | **2** — *raised from `[?]` on 2026-09-10, [`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) F9, and it is **behaviour rather than an account**: no receiver has said anything, and four of them were watched doing it.* `✓` In the 214 forks round 3 read, **at least four independent receivers touch secrets in inherited material as one of their first commits**: [`insurgently/dotfiles`](https://github.com/insurgently/dotfiles) titles a commit **“remove encrypted files”** and strips `.key.txt.age`, an encrypted VPN credential, the author's GPG trustdb and an encrypted SSH private key; `trang-kaleido/kaleid.mvp-claude` titles one **“Re-committing work without secrets”** and then adds the 40-line `.env.example` the material did not ship; two more rewrite an API key and an `.env.mcp`. **A 2: it costs them a cleanup, and in one case a history rewrite. Not a 3 — nobody is on record changing how they work.** Prior: `.env.example` exists for the receiver, and **no receiver had said anything about it** | **`[?]`** | **§5** `needsEnv` · **§6** — collected across the resolved set and written to `.env.example` before export | **The runtime half is occupied; the handover half is not.** *Narrowed 2026-09-09 on [`re-research.md` round 2](../6-personas/re-research.md#round-2-counting-not-asking) Q-D — this cell read "No, in this space", which J-70 and D-9 showed was too strong.* What exists: a credential-injection proxy outside the sandbox, a vault with time-scoped access, a key broker that injects into the shell, a WASM sandbox with a secret scanner, *"block secrets"* policies. **Every one of them keeps a key away from a *running* agent; none is about what a set carries when it leaves** — and a person names our exact failure: *"a skills file can just easily say '**We connect to bob using key Z and user X**'"*. `M` Vercel's env drawer is still the best-observed prior art in the phase (flow 07) and belongs to a different product. **Doppler and Infisical are captured in flow 07 and still unscored — a gap in the benchmark, one afternoon, and round 3 did not spend it either.** **What round 3 did find belongs in this row and points the other way down the pipe** (Q-F, 2026-09-10): handed an archive whose `.env.example` named a key with no value, one of three receiving agents **read the machine's live OAuth token out of the `gh` keyring and wrote it into a plaintext file**, unprompted, having been told only to set the project up. Another refused and said why. **The persona columns do not move — a receiving agent is a machine and P2 is a person** — but the risk under this job is now known to run in both directions: not only what the sender packs, but **what the receiver fetches to fill the hole the sender left.** `.env.example` is an instruction to go and find the value |
| **EJ-1 — not be quietly overruled by my own tools** | **3** — `✓` [#20412](https://github.com/anthropics/claude-code/issues/20412) **142**, *"silently synced… without any opt-in, notification, or consent"* · `✓` [codex #13386](https://github.com/openai/codex/issues/13386), dropped *"with no warning anywhere"* · `✓` #1219 · `*` Q13, Q14 — *"my guesses are unfalsifiable"* | **2** — `*` Q21: overruled by **absence** — half his tools missing, and nothing said. **Second-hand** | **`[?]`** | **§6** — three severities, *nothing blocks*, the unclean export **confirmed** with its consequence in the present tense, and *Skipped* given a glyph it earned · **§7** — state 6 naming the fields that differ | **The market has the mechanism and the nearest competitor failed at it.** `M` Port ships pass / warn / block with the reason attached; GitHub's mergebox names consequences. `E` **Continue shipped `ConfigValidationError { fatal: boolean }` and filed a block that failed to resolve as `fatal: false`** — the exact failure, in our space, in production. **And it is no longer only a post-mortem: round 3 produced two live specimens** (Q-J, 2026-09-10). `asm audit` calls two files with different sha1s *"identical copies"* and offers to delete one. `asm audit security pdf` returns **`SAFE`** on a scan of **`0 files`** and **`0 lines`** — **a green verdict on a scan that had nothing to scan**, which is precisely what §6 gives *Skipped* its own neutral glyph to prevent. **The importance stays 3 and the decisions this row justifies now have dated evidence rather than a single dead competitor** |
| **EJ-2 — believe a clean result was earned** | **2** — *restored 2026-09-10 on new evidence, [`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) F2; the withdrawal below was correct on what it had.* `✓` [Cursor forum #144731](https://forum.cursor.com/t/rules-not-being-applied-as-expected/144731), 457 views: a named person builds a **minimal reproduction repository**, chases it for ten days, calls the rules system *“rather unusable”*, and asks for the one thing this job is about — *“it would **Really** be useful if we could **inspect the raw full context window**… to tell if it is an issue with **AI not loading the rule vs just deciding not to follow them**”*. **The vendor confirms the ground**: *“rules apply inconsistently… even `alwaysApply: true` sometimes doesn’t apply automatically on the first request”*. **A 2 and not a 3: it cost him, and he did not change how he works — he asked to be shown.** **This is also the sharpest public statement of Q12 anybody has made, and Q12 no longer stands on one person.** *Withdrawn 2026-09-09, J-74: the cell stated that its own evidence carries no weighting and then held a 2.* The utterances stand: `✓` R7: *"**blackbox oracles make bad workflows, and tend to produce a whole lot of cargo culting**"* ([bandrami](https://news.ycombinator.com/item?id=46820441)); *"engineers will always invent weird heuristics to detect drift"* ([Alpha_Logic](https://news.ycombinator.com/item?id=46106973)). **HN comment scores are not exposed, so this evidence carries no weighting at all** | **`[?]`** | **`[?]`** — arguably central for someone taking strangers' material; arguable is not evidence | **§6** — *Skipped* gets a neutral glyph rather than a tick it did not earn · **§5** — no score, no rating, no badge | **They close a different job.** Tessl scores 93 with an uplift multiplier, Smithery out of 100, `HarnessKit` claims 0–100 (`V`). Those answer *give me a signal*; whether a produced number makes anyone **believe** is `[?]`, and reading their scores as a failure at *this* job is **our inference, `R`**. **Round 3 makes half of that inference a measurement and leaves the other half exactly where it was.** `✓` What the free tier actually produces, run on our set: `asm eval` scores the `pdf` skill **61/100 (D)** across ten categories, giving *Structure & completeness* **8/10** to an item missing every file it instructs the agent to read; `ai-agent-skills validate` returns **PASS · Skill is valid** on the same item. **Green ticks that were not earned, produced by tools, observed.** `[?]` **Whether any of that makes a person believe anything is still unknown, and no instrument in this round could reach it** — so the cell keeps its withdrawn number |
...
| **SJ-1 — not be the missing manual for my own work** | **2** — `*` Q21, Q22: *"which files are load-bearing and which are aspirational… **none of that is written down anywhere**"* · `✓` that other people do open this material — #6235 6,592, [#10238](https://github.com/anthropics/claude-code/issues/10238) 168, [#28729](https://github.com/anthropics/claude-code/issues/28729) 151 | **2** — *raised from `[?]` on 2026-09-10, [`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) F10, on the same fork corpus and again as behaviour.* `✓` **30 of 214 receivers — 14% — wrote a setup, handover or installation manual the material they received did not have, 21,266 lines of it in total.** They name it themselves: [`HANDOFF.md`](https://github.com/noelsaw1/claude-code-dotfiles-1) at 229 lines **as the fork's only commit**, another `HANDOFF.md` at 1,041, a `SETUP.md` at 425, `INSTALL-GUIDE.md` at 283, `docs/windows-setup.md` at 284, `mcp-setup.md`, `developer-setup.md`, `cross-machine-claude-setup.md`, and one titled *“Make setup.sh safe for complex pre-existing installs”* beside a 120-line note on adapting the install to a machine that already had things. **A 2: hours of writing, repeatedly, to supply what the sender did not.** The receiver's other half is **H-J7**, which still stands on one aside | **`[?]`** | **§6** — `SETUP.md` addressed to the agent that opens the project, stating per item what it requires | **No — and, like RJ-1, the *structural* half of that is withdrawn.** *Updated 2026-09-10, round 3.* `asm doctor` and `ai-agent-skills doctor` both describe the receiving machine, so the gap is not that nobody can; it is that nobody does it **for a named set**. **And this row's feature cell was run**: three receiving agents read a `SETUP.md` of exactly the shape §6 describes and performed the setup from it, which is the closest thing to *not being the missing manual* that has ever been tested here. **The catch belongs in the same cell:** the document told them what each item requires and **not what was wrong with the set**, and two of the three then either mis-resolved a defect or missed every one |
...
|---|---|---|---|---|---|
| **H-J1 — lay hands on something I wrote months ago** | **2** — *raised from `[?]` on 2026-09-10, [`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) F8.* `✓` [Cursor forum #148996](https://forum.cursor.com/t/prompt-library-notepads-2-0/148996): a Cursor user whose prompt library **is a folder of notes in Telegram** — *“I have several templates in my notes in Telegram, and I have to copy them into new chats, **as well as manually enter each link to each file, command, or skill** that I want to use”* — and who checked whether skills solved it and answered himself *“No, that's not it”*. `✓` Corroborated in an adjacent population by [a full specification of the missing thing](https://community.openai.com/t/a-built-in-prompt-vault-or-prompt-library-added-to-chatgpt-would-be-extremely-useful/1390442) — save, name, **folders, tags, search**, edit, duplicate, variables — answered by the vendor with *“no timeline to share”*. **A 2: manual re-entry every time is a cost; neither changed how they work.** Prior: one sighting, `*` Q19: *"it usually takes longer than rewriting it would"*. Evidence that it **happened**, not of how much it matters | **`[?]`** | **`[?]`** | **§8** — Library, filters by kind and tag, search · **§5** — per-item usage facts | **Yes, and well.** B1 is where the craft is concentrated — **Linear, GitHub code search, Obsidian and VS Code are the cells that were actually scored**; Backstage's catalog and Raycast's store were surveyed in stage 1 and never benchmarked (corrected 2026-09-09, J-84). **We would be competing where the market is strongest and our evidence thinnest** |
...
| **H-J3 — change one copy for one project without touching the rest** | **2** — *raised from `[?]` on 2026-09-10, [`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) F3.* `✓` [Cursor forum #147236](https://forum.cursor.com/t/rules-in-home-folder-cursor-rules-are-not-applied/147236), 396 views: blocked three ways at once — the documented home-folder route is a confirmed bug, his organisation forbids the vendor’s team rules, and committing to the repo gives no per-user variation — he states the requirement in his own words: *“I need share the rules with my team… **each user should be able to slightly modify the rules**”*. **One shared source with a per-user modified copy is `detached` and `overrides` in §5**, described by somebody who has no mechanism for it. **A 2: it costs him the thing entirely, and there is no sign he changed how he works.** **J-107 said this stood on nothing; it no longer does.** Prior: one aside, `*` Q26: *"my rule says one thing, the client's linter says another, and **there's no precedence anywhere**"*. That is **Q10**, and it is a conflict with something **outside** the set entirely, which §5 has nowhere to put | **`[?]`** | **`[?]`** | **§5** — `detached`, `overrides`, promotion as a new item · **§7** — state 6, with the differing fields named | **Half.** `M` Figma models overrides precisely enough to offer *Reset fill* by name — **and erases the origin at detach, so there is no way back** (flow 05). Our return path has **no prior art in the survey**, which cuts both ways. **Round 3 measured the state this job addresses without touching its importance** (Q-H): copies of one item do diverge — 14% of 7,506 duplicated items are out of sync in published trees — and **they stay diverged**, median 121 days, with reconciliation essentially never observed. **That makes the mechanism's target real and the demand for it no less `[?]` than before.** It also makes intent unreadable: with nobody ever reconciling, there is no behavioural signature separating *a copy somebody meant to keep different* — which is this job — from *drift nobody noticed*, which is RJ-3 |
| **H-J4 — get moving with material somebody else made** | **`[?]`** | **`[?]`** | **`[?]` as an importance — and no longer `[?]` in both directions.** *Updated 2026-09-09 from [`re-research.md` round 2](../6-personas/re-research.md#round-2-counting-not-asking) Q-E — the only cell in this matrix that gained evidence, and it argues against the feature.* **Four of the first five practitioners ever observed on installing other people's material refuse it, minimise it, or prefer their own**; the one positive was distributing **his own** work through a marketplace mechanism, which is P1's job. What survived is *"a few established names"* — **provenance over volume**. **Nobody in P3's shape has still ever been asked anything, so there is no number here and cannot yet be one.** **Updated again 2026-09-10 (round 3, Q-I), and this is the largest change of evidence anywhere in the matrix: the cell now has opinion on one side and behaviour on the other.** `✓` Five well-known public skills are carried **verbatim** by 2,040 to 4,024 repositories each; of **446** foreign copies read in full, **143 — 32% — were edited after the day they were added**, at a **median of 27 days** (only 17% same-day, so it is use rather than adaptation-at-install), and **45 — 10% — name their origin anywhere in the file**. On npm, `npx skills` went from **19 downloads a month** in December 2025 to **44.4 million** in July 2026. **The importance is still `[?]` and must stay `[?]`**: none of this says who any importer was, still less that they were empty-handed. **What it removes is the reading that the shelf's premise describes nothing anybody does** — round 2 measured what five people say, round 3 measured what thousands of repositories contain, and *both stand* (rule 4). **What it sharpens is the same thing round 2 said**: provenance is what nine copies in ten lose, so showing it is the part that has to be ours | **§8** — the `My library` / `Public library` scope switch · **§11** — the curated read-only shelf and the example project | **Yes — the most crowded space in the entire survey, and as of 2026-09-09 the supply is counted rather than quoted.** `✓` **19,703 repositories** match `"claude skills"` in name or description, topped by curated collections at **74,686★**, 25,709★ and 15,006★ — against **93** public personal agent-material repositories (Q-E). `V` Smithery 17,500 servers, Tessl 3,000+ skills, Agentman 115 business skills, Notion's template gallery, plus the whole installer category. **Every catalog in the phase solves cold start this way** (OBS-12) |
...
| **H-J7 — the unwritten half travels too** | **2** — *raised from `[?]` on 2026-09-10, [`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) F12.* `✓` **Four independent people describe the same cost in the first person**, and one prices it: *“you lose context, lose progress, **re-explain everything, and waste 10–30 minutes every time**”*; *“every session I'd **re-explain the architecture, re-discover the same bugs, re-learn the same solutions**. `CLAUDE.md` helps but it's manual”*; *“every time I switch tools/models, I have to re-explain the project. **Specs live in my head** or in random chat history”*; and a six-year lead developer building a handover package because *“all the institutional knowledge **lived in my head**”*. **A 2 and deliberately not a 3.** All four also built software about it — **and they were found in a venue where having built something is the entry ticket**, so counting the building as importance would be circular. The stated cost is what the cell rests on. **Their posts scored 1–3 points**, so the venue's own weighting is worth nothing here either (J-74's rule). **Q11 no longer stands on one aside.** | **`[?]`** — `*` Q26, one aside, and it is the **receiver's** half of this job: *"roughly **half** of what makes a project go well is stuff I've never written down"*. **If it holds it bounds the ceiling of the product**, because we validate the written part. **The four voices in the P1 cell are all authors**, so this cell is untouched by them | **`[?]`** | **Nothing, and nothing can be** | **No.** Nobody in the survey attempts it |
...
one; *the only row with a number in both* was wrong (J-91). **Market: nobody above 4 on B4** — and the
*structural* explanation is withdrawn. *Corrected 2026-09-10, round 3 Q-J.* This said **no candidate
has a surface that says anything about the machine its artefact lands on**, which was measured on
funded vendors in August 2026; `asm doctor`, installed and run, checks git and its version, `gh`
authentication, the Node version, 21 provider directories, lock-file integrity, disk space and PATH
shadowing. **The open cell is narrower and firmer than it was: nobody describes the receiving machine
*for a named set*.** **The spec already spends most of its handover budget here** — §6's `SETUP.md`
decision, the target selector, the disclosure stages — which this row confirms rather than discovers,
**and which Q-F has now watched work**: three receiving agents out of three performed the setup, while
two of the three did nothing useful with the defects the document did not name.
...
| **Promote a detached item** (§5, §7) | **H-J3** | **Nothing.** *Corrected 2026-09-09 (J-107):* this read *one aside, `*`* — but that aside is about a conflict with something **outside** the set (the client's linter, Q10), which is not a request for a per-project override. **Nobody has said anything about wanting one** | Q4's disposition already noted that promotion only has value if in-project editing exists. ~~**Both stand on nothing rather than on one aside, and they stand or fall together.**~~ **Corrected 2026-09-10 ([`re-research.md` round 4](../6-personas/re-research.md#round-4-the-matrixs-empty-cells) F3): they no longer stand on nothing.** A person on Cursor's forum, blocked three ways at once, states the mechanism as his requirement — *“I need share the rules with my team… **each user should be able to slightly modify the rules**”* — which is one shared source plus a per-user modified copy, i.e. `detached` and `overrides`. **One person outside this project, in public, unprompted, describing the mechanism rather than the feature.** H-J3's P1 cell is now a **2**. Flow 05 found no prior art for the return path — so the one mechanism in the spec that is invented rather than copied is also the one with no demand behind it (D-6) |
| **Library-wide search and filters** (§8) | **H-J1** | One sighting, `*` | **The awkward one.** It is the market's best-served flow (B1) **and** our thinnest evidence **and** the thing `CLAUDE.md` §8's accepted cost was priced against — a cost priced at **300 items**, which nothing supports (R4). **The replacement figure is now counted rather than blended**: four public agent-material repositories hold **11, 25, 47 and 48 items**, two of them above 40 ([`re-research.md` round 2](../6-personas/re-research.md#round-2-counting-not-asking) Q-C). *Corrected 2026-09-09 (J-108): this said "build it for 20–40", a range made of one organisation's threshold and one person's file count.* **Tens, not hundreds — design for fifty**, and note the instrument only sees people who publish |
...
[`re-research.md` round 2](../6-personas/re-research.md#round-2-counting-not-asking), the targeted round the audit asked for.
...
**What is reachable instead is written down as [`re-research.md` round 3](../6-personas/re-research.md#round-3-five-questions-that-need-no-interview)** —
