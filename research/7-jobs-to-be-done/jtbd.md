...
**[`re-research-4.md`](../6-personas/re-research-4.md) went after the persona columns directly**, in
...
**[`re-research-3.md`](../6-personas/re-research-3.md) ran five questions and changed not one
...
| **Promote a detached item** (§5, §7) | **H-J3** | **Nothing.** *Corrected 2026-09-09 (J-107):* this read *one aside, `*`* — but that aside is about a conflict with something **outside** the set (the client's linter, Q10), which is not a request for a per-project override. **Nobody has said anything about wanting one** | Q4's disposition already noted that promotion only has value if in-project editing exists. ~~**Both stand on nothing rather than on one aside, and they stand or fall together.**~~ **Corrected 2026-09-10 ([`re-research-4.md`](../6-personas/re-research-4.md) F3): they no longer stand on nothing.** A person on Cursor's forum, blocked three ways at once, states the mechanism as his requirement — *“I need share the rules with my team… **each user should be able to slightly modify the rules**”* — which is one shared source plus a per-user modified copy, i.e. `detached` and `overrides`. **One person outside this project, in public, unprompted, describing the mechanism rather than the feature.** H-J3's P1 cell is now a **2**. Flow 05 found no prior art for the return path — so the one mechanism in the spec that is invented rather than copied is also the one with no demand behind it (D-6) |
